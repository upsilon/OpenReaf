...
class monthlyAction extends Action
{
	...
	function execute()
	{
		...
		$j = date('w', $curMonth);
		for ($i = 0; $i < $j; ++$i) {
			$recs[$i] = array('month' => $Month, 'day' => '', 'wday' => $i);
		}
		...
		for ($i = 1; $i <= $last_day; ++$i) {
			...
			$tmpArr = array('month' => $Month, 'day' => strval($i), 'wday' => $j%7);
		...
		if ($tmpCount > 0) {
			for ($i = $tmpCount; $i < 7; ++$i) {
				$recs[$j] = array('month' => $Month, 'day' => '', 'wday' => $j%7);
				++$j;
			}
		}
		...
		if (_TermClass_ == 'Mobile') {
			$strYM = date(OR_MONTH_FORMAT, $curMonth);
		} elseif (_LANGUAGE_ != 'ja')  {
		...
		if ($_SESSION['SentakuMode'] == 2) {
			$BackLink = '?op=preset';
		} elseif (isset($_SESSION['skip_stj'])) {
