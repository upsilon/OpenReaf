...
class historyAction extends Action
{
	...
	function execute()
	{
		...
		if (isset($_POST['sort'])) {
			...
			$this->sort = $_SESSION['HISTORY']['sort'];
		}
		$PageNO = 0;
		if (isset($_GET['page_no'])) {
			$PageNO = intval($_GET['page_no']);
		} elseif (isset($_SESSION['HISTORY'])) {
			$PageNO = $_SESSION['HISTORY']['page_no'];
		}
		$_SESSION['HISTORY']['sort'] = $this->sort;
		$_SESSION['HISTORY']['page_no'] = $PageNO;

		$recs = $this->get_reserve_list($_SESSION['UID']);
		...
		{
			$set = ($i == $PageNO) ? 1 : 0;
			$pages[] = array('pagenum' => $i+1, 'url' => 'index.php?op=history&page_no='.$i.'&sort='.$this->sort, 'set' => $set);
		}
		$nextpage = '';
		if (($PageNO + 1) < $max_page && $max_page > 1) {
			$nextpage = 'index.php?op=history&page_no='.($PageNO+1).'&sort='.$this->sort;
		}
		$prevpage = '';
		if ($PageNO > 0) {
			$prevpage = 'index.php?op=history&page_no='.($PageNO-1).'&sort='.$this->sort;
		}
		...
		$condition = OR_HISTORY_LIST.' :: 【'.$_SESSION['UNAME'].'】';
		...
		$this->oSmarty->assign('op', 'history');
	...
	function get_reserve_list($UID)
	{
		...
		$aWhere = array(_CITY_CODE_, $UID, date('Ymd', strtotime('-2 month')), _CITY_CODE_, $UID, date('Ymd', strtotime('-2 month')));
		$sql = "SELECT DISTINCT y.pulloutyoyakunum as yoyakunum,
			...
			WHERE y.localgovcode=? AND y.userid=? AND y.usedate>=?";
		if (_LIST_RESERVE_ONLY_) {
			$sql.= " AND y.pulloutjoukyoukbn='4'";
		} else {
			$sql.= " AND DATE(y.usedate)<DATE(NOW()) AND y.pulloutjoukyoukbn<>'3'";
		}
		$sql.= " UNION SELECT DISTINCT y.pulloutyoyakunum as yoyakunum,
			...
			WHERE y.localgovcode=? AND y.userid=? AND y.usedate>=?";
		if (!_LIST_RESERVE_ONLY_) {
			$sql.= " AND DATE(y.usedate)<DATE(NOW())";
		}
		...
		$aWhere = array(_CITY_CODE_, $UID, date('Ymd', strtotime('-2 month')), _CITY_CODE_, $UID, date('Ymd', strtotime('-2 month')));
		$sql = "SELECT DISTINCT y.yoyakunum, y.honyoyakukbn,
			y.shinsakbn, y.escapeflg,
			y.shisetsucode, y.shitsujyocode, y.combino,
			...
			AND y.usedatefrom>=? AND DATE(y.usedatefrom)<DATE(NOW())
			UNION SELECT DISTINCT y.yoyakunum, y.honyoyakukbn,
			y.shinsakbn, y.escapeflg,
			y.shisetsucode, y.shitsujyocode, y.combino,
			y.usedatefrom as usedate, y.usetimefrom, y.usetimeto,
			s.shitsujyokbn, s.shitsujyoname,
		...
		if (!_LIST_RESERVE_ONLY_) {
			$sql.= " AND DATE(y.usedatefrom)<DATE(NOW())";
		}
		$sql.= " ORDER BY yoyakunum, shitsujyokbn, shitsujyocode";
		$res = $this->con->getAll($sql, $aWhere, DB_FETCHMODE_ASSOC);
