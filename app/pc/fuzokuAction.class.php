...
class fuzokuAction extends Action
{
	...
	function execute()
	{
		...
		if (isset($_POST['chkClicktime'])) {
			...
			foreach ($_POST['chkClicktime'] as $i => $val)
			{
				if ($val == '0') continue;

				$key = $_POST['KomaKbn'][$i];
				$_SESSION['Y_I']['UTFrom'][$key] = $_POST['hdntimefrom'][$i];
				$_SESSION['Y_I']['UTTo'][$key] = $_POST['hdntimeto'][$i];
			}
		}
		...
		if (count($recs) == 0) {
			header('Location: index.php?op=mokuteki');
			exit();
		}

		$komaCount = count($recs);
		$htmlTdCount = 4;
		$tData = array();
		...
		$this->oSmarty->assign('UseTime', $UseTime);
