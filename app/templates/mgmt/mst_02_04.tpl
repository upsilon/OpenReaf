...
<h2 class="subtitle01">固定減免率</h2>

<div align="center">
{if $errmsg}<div id=errorbox>{$errmsg}</div>{/if}
<form name="forma" method="post" action"index.php">
<input type="hidden" name="op" value="mst_02_04_exemption">
...
{foreach $results as $key => $value}
  <tr align="center">
    ...
    <td>{$value.koteigencode}<input type="hidden" name="Code[{$key}]" value="{$value.koteigencode}"></td>
    <td><input type="text" size="30" maxlength="64" name="CodeName[{$key}]" value="{$value.koteigenname}" style="ime-mode:active;"></td>
